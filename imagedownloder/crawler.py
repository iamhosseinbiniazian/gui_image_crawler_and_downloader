...
def google_image_url_from_webpage(driver,logger,numImage):
    ################################################################################################3
    image_urls = list()
    ctr=1
    waittime=5
    while len(image_urls)<numImage:
        image_elements = driver.find_elements_by_class_name("rg_l")
        url_pattern = "imgurl=\S*&amp;imgrefurl"
        logger.log(logging.INFO,"num crawl image=%s"%(str(len(image_urls))))
        for image_element in image_elements:
            outer_html = image_element.get_attribute("outerHTML")
            # logger.log(logging.INFO, "num crawl image=%s" % (str(len(outer_html))))
        ...
        try:
            ...
            doc_height = driver.execute_script(
                "window.scrollTo(0,"+str(val)+ "*document.body.scrollHeight);var lenOfPage=document.body.scrollHeight;return lenOfPage;")
...
def bing_gen_query_url(keywords, face_only=False, safe_mode=False):
    ...
    if not safe_mode:
        query_url += '&safesearch='
        query_url+='off'
    return query_url
def bing_image_url_from_webpage(driver,numImage=500,safemode=False):
    image_urls = list()
    ctr=1
    waittime=5
    time.sleep(3)
    ...
    time.sleep(3)
    if not safemode:
        image_elements = driver.find_elements_by_class_name("ftrLi")
        if len(image_elements)!=0:
            image_elements[1].click()
            safeoff=driver.find_element_by_class_name('ftrD_MmVert')
            safeoff=safeoff.find_elements_by_tag_name('a')
            safeoff[1].click()
    time.sleep(3)
    while len(image_urls)<numImage:
...
def crawl_image_urls(keywords,logger, engine="Google", max_number=100000,
                     face_only=False, safe_mode=False, proxy=None, 
                     proxy_type="http", quiet=False, browser="phantomjs",numImage=500):
    """
    ...
    logger.log(logging.INFO,"Scraping From {0} Image Search ...\n".format(engine))
    # my_print("Keywords:  " + keywords, quiet)
    logger.log(logging.INFO,"Keywords:  " + keywords)
    if max_number <= 0:
        logger.log(logging.INFO, "Number:  No limit")
        # my_print("Number:  No limit", quiet)
        max_number = 100000
    else:
        # my_print("Number:  {}".format(max_number), quiet)
        # messagebar.emit("Number:  {}".format(max_number))
        logger.log(logging.INFO, "Number:  {}".format(max_number))

    # my_print("Face Only:  {}".format(str(face_only)), quiet)
    # messagebar.emit("Face Only:  {}".format(str(face_only)))
    # my_print("Safe Mode:  {}".format(str(safe_mode)), quiet)
    # messagebar.emit("Safe Mode:  {}".format(str(safe_mode)))
    ...
    # messagebar.emit( "Query URL:  " + query_url+'\n')
    logger.log(logging.INFO,"Query URL:  " + query_url+'\n')
    ...
    if engine == "Google":
        ...
        image_urls = google_image_url_from_webpage(driver,logger,numImage=numImage)
    elif engine == "Bing":
        driver.set_window_size(1920, 1080)
        driver.get(query_url)
        image_urls = bing_image_url_from_webpage(driver,numImage=numImage,safemode=safe_mode)
    else:   # Baidu
